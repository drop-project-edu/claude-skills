...
## 6. Hand it over, inactive

A validated assignment is still inactive, and this is where the skill stops. **Do not activate it on
your own**, not even when the report comes back clean: when students may start submitting is the
teacher's decision, and an assignment opened early collects submissions they didn't ask for.

Confirm with `get_assignment_info`, give the teacher the assignment's url, and tell them it is ready
but not yet open. Call `set_assignment_active` with `active: true` only when they ask for it, now or
later. Activation is refused while the report has errors or while the assignment has never been
connected, which is the intended safety net: students would otherwise submit to an assignment Drop
Project can't evaluate.
